export default class ThemeSwitch {
  constructor(switchersArr) {
    this.switchersArr = switchersArr;
  ...
  _setThemeSwitchers(isDark) {
    if(isDark) {
      this.switchersArr.forEach(switcher => switcher.checked = true)
    } else {
      this.switchersArr.forEach(switcher => switcher.checked = false)
    }
  };
  ...
  _setEventListeners() {
    this.switchersArr.forEach(switcher => {
